...
class Emr_rec:
    ...
    def set_loop(self):
        ...
        for loop_count in (range(5)):
            ...
            #################################################trigger方法設定箇所#################################################
            #emr_trigger = self.video_writer_sensor(emr_trigger, writer) #センサーtrigger時はこちらをコメントアウト
            emr_trigger = self.video_writer_diff(emr_trigger, writer) #image差分動体検知trigger時はこちらをコメントアウト
            ####################################################################################################################
    ...
    def video_writer_sensor(self, emr_trigger, writer):
        ...
        while delta <= interval:
            ...
            t1=datetime.datetime.now()
            delta = t1 - t0
            tr_signal = senser.motion_detect() # センサーの物体検知有無を確認
            if tr_signal == 1: 
